...
const NETWORK_CONFIG = {
    contractAddress: "0x322e1eA697c24917eC647Cc5be508B083539dA47",
...
export async function voteeNo(proposalId: any, amount: any) {
    try {
        ...
        const voteNoContractAddress = baseWeb3.utils.toChecksumAddress(
            NETWORK_CONFIG.contractAddress
        );

        const voteNoContract = new baseWeb3.eth.Contract(
            votingAbi,
            voteNoContractAddress,
        ...
        const buyNoTxGas = await voteNoContract.methods
            .buyNoShares(proposalId, amount)
            .estimateGas();

        const buyNoTx = await voteNoContract.methods
            .buyNoShares(proposalId, amount)
            .send({ gas: Math.floor(buyNoTxGas * 1.2) });

        console.log("Waiting for Buy transaction in Fhenix...");
        const receiveTxReceipt = await waitForTransaction(
            baseWeb3,
            buyNoTx.transactionHash
        );
        if (!receiveTxReceipt) {
